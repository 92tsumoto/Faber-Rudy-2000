...
void current(FILE *fp4, FILE *fp5, FILE *fp6, FILE *fp9, double t, double u[])
{
...
{
	fprintf(fp6,"%e %e %e %e %e %e %e\n",time,var.ilca,p[4],p[5],var.fca,var.ilcana,var.ilcak);
...
void out_ikr (FILE *fp4, double time, double p[])
{
	...
	fprintf(fp4,"%lf %lf %lf %lf\n",time,var.ikr,var.ikr_without,var.ikr_with);
...
void out_cicr (FILE *fp9, double time, double p[])
{
	...
	fprintf(fp9,"%e %e %e\n",time,var.Irel_cicr,var.Irel_jsr_overload);
