...
main(int argc, char **argv)
{
	...
	for (ii = 0; ii < var.datas; ii++){
		...
		while(1){
			...
			apd = 0.0; toneapd = 0.0; ttwoapd = 0.0; rmbp = x[0]; vmax = -90.0; dvdtmax = 0.0, v_old = x[0]; var.boolien = 0;
			...
			for (j = 0; j< (int)tt; j++){
				...
				if (var.pswitch==1){
					if(j%10==0){
						...
						if(var.out_data){
							current(fp4,fp5,fp6,fp9,ttime,x);
						}
					}
				}
		...
		if(var.write0){
			fclose(fp4); fclose(fp5); fclose(fp6); fclose(fp7); fclose(fp9);
		}
